<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for PokeMMO</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the Archetype PokeMMO theme</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

1. Clone this repo to your PokeMMO mods folder (or replace the `CHOOSE_YOUR_COLORS.xml` in your existing Archetype theme folder)
2. Go to `Mod Management` and enable `Archetype Theme` and restart PokeMMO.
3. Reopen PokeMMO, go to Settings > Interface > Theme and select `Archetype` from the themes dropdown.
4. Restart PokeMMO and enjoy!

## Gallery

![Login Screen](https://raw.githubusercontent.com/trashcatt/rose-pine-pokemmo/refs/heads/beta-lny25/assets/login.png)
![Overworld](https://raw.githubusercontent.com/trashcatt/rose-pine-pokemmo/refs/heads/beta-lny25/assets/overworld.png)
![Dialog](https://raw.githubusercontent.com/trashcatt/rose-pine-pokemmo/refs/heads/beta-lny25/assets/dialog.png)
![Battle](https://raw.githubusercontent.com/trashcatt/rose-pine-pokemmo/refs/heads/beta-lny25/assets/battle.png)
## Thanks to

- [ssjshields](https://github.com/ssjshields) for the Archetype theme. (All I did was change some colors.)
